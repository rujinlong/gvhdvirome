...
fout15_svg <- path_target("PCOA_TIME_TRY123_NEW.svg")
...
pcoa <- data.table::fread(fout14) %>% 
    dplyr::mutate(group = str_replace(group, "Day", "d")) %>%
    dplyr::mutate(PCoA2 = PCoA2 * -1) # PcoA2 axis flipped due to package or R version. To make the figure consistant, we flip it back
plot.theme = theme(
    plot.title = element_text(size = 18, color = "black", family = "Arial", face = "plain", vjust = 0.5, hjust = 0.5),
...
P1 <- ggplot(pcoa, aes(PCoA1, PCoA2, group = group, color = group)) +
    ...
    geom_point(size=2) +
    scale_color_manual(values = c("d -7" = "#edf8fb", "d 0" = "#bfd3e6", "d +7" = "#9ebcda", "d +14" = "#8c96c6", "d +21" = "#8856a7", "d +28" = "#810f7c")) +
    # scale_color_manual(values = c("d -7" = "black", "d 0" = "black", "d +7" = "black", "d +14" = "black", "d +21" = "black", "d +28" = "black")) +
  ...
  # scale_fill_manual(values = "black") +
    ...
    stat_ellipse(aes(color = group), level = 0.4, linetype = 2, fill = group) +
...
p2 = P1 + plot.theme
p2
ggsave(fout15_svg, p2, width = 1600, height = 1200, units = "px")
...
fout22_svg <- path_target("PCOA_ABX_patient_NEW.svg")
...
# dplyr::mutate(Axis.1 = Axis.1 * -1)  # PcoA1 axis flipped due to package or R version. To make the figure consistant, we flip it back
...
p1
ggsave(fout22_svg, p1, width = 1600, height = 1200, units = "px")
```
...
fout32_svg <- path_target("PCOA_GVHD_patient_NEW.svg")
...
p1 = pcoa_plot + plot.theme
p1
ggsave(fout32_svg, p1, width = 1600, height = 1200, units = "px")
```
...
p2 = P1 + plot.theme
p2
ggsave(fout75_svg, p2, width = 1600, height = 1200, units = "px")
```
...
pcoa_plot <- ggplot(pcoa, aes(PCoA1, PCoA2, group = group, color = group)) +
    ...
    geom_point() +
    scale_color_manual(values = c(" No Antifungals" = "#FFB562", "Antifungals" = "#F9F2ED")) +
...
fout24_svg <- path_target("PCOA_AF_patient_NEW_1.svg")
...
pcoa_plot <- ggplot(pcoa, aes(PCoA1, PCoA2, group = group, color = group)) +
    stat_ellipse(aes(colour = group), level = 0.95, linetype = 2) +
    geom_point() +
    scale_color_manual(values = c("MUC No-antifungals" = "#8dd3c7", "REG No-antifungals" = "#ffffb3", "MUC antifungals" = "#bebada", "REG antifungals" = "#fb8072")) +
...
p2 = p1 + xlab(paste("Axis1 ", 5.78, "%", sep = "")) + ylab(paste("Axis2 ", 5.24, "%", sep = "")) 
p2
ggsave(fout24_svg, p2, width = 2400, height = 1200, units = "px")
```
