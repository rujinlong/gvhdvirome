...
P1<-ggplot(pcoa, aes(PCoA1, PCoA2, group = group, color = group))+
  ...
  geom_vline(xintercept = 0,lty="dashed")+
  geom_hline(yintercept = 0,lty="dashed")+
  labs(x=paste0("Axis1 ",5.0,"%"),
       y=paste0("Axis2 ",4.6,"%"))+
  geom_point()+scale_color_manual(values =c("d -7"="#edf8fb","d 0"="#bfd3e6", "d +7"="#9ebcda","d +14"="#8c96c6","d +21"="#8856a7","d +28"="#810f7c"))+
  ...
  stat_ellipse(aes(color = group),level=0.4,linetype = 2,fill=group)+theme(legend.position="none")
P1
plot.theme = theme(plot.title=element_text(size=18, color="black", family  = "Arial", face= "plain",vjust=0.5,hjust=0.5),
                   axis.line=element_line(size=.5, colour="black"),
                   axis.ticks=element_line(color="black"),
...
pcoa_plot <- ggplot(pcoa, aes(PCoA1, PCoA2, group = group, color = group))+
  stat_ellipse(aes(colour = group),level=0.95,linetype = 2)+
  geom_point()+scale_color_manual(values =c("MUC No-antifungals"="#8dd3c7","REG No-antifungals"="#ffffb3","MUC antifungals"="#bebada","REG antifungals"="#fb8072"))+
