...
fout_abx <- path_target("Alpha-InvSimpson-5D-Tall.svg")
...
p1
p2<-p1+stat_compare_means(comparisons=my_comparisons, label.x = 1.5, label.y = 25)
p2
p3<- ggpar(p2, ylim = c(0, 30))
p3

ggsave(fout_abx, p3, width =700,height = 1400,units = "px" )
```

```{r}
#| message: false
fin_abx <- here(path_data, 'Alpha_ABX_Final.txt')
fout_abx <- path_target("Alpha-Richness-5D-Tall.svg")
otu <- read.delim(fin_abx, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)
# theme_bw() +
#   theme(axis.line = element_line(color='black'),
#         plot.background = element_blank(),
...
p0<-ggboxplot(otu,
              ...
              add = "jitter")
p0
p1<-p0+color_palette(c("#0F7FFE", "#FB0106"))+labs( y = "Richness",x="")+theme(legend.position = "none", axis.text.x=element_text(family="Arial",colour="black",angle = 45,vjust = 0.15,hjust = 0.5))
p1
p2<-p1+stat_compare_means(comparisons=my_comparisons, label.x = 1.5, label.y = 3500)
p2
p3<- ggpar(p2, ylim = c(0, 4000))
p3
...
p3<- ggpar(p2, ylim = c(0, 30))
p3
png(file = fout_1e, width =600, height = 1400,units = "px",res =300) 
dev.off()
```
...
fout_gvhd <- path_target("Alpha-Richness-5A-TALL.svg")
...
p2<-p1+stat_compare_means(comparisons=my_comparisons, label.x = 1.5, label.y = 2000)+theme(axis.text.x=element_text(family="Arial",colour="black",angle = 45,vjust = 0.15,hjust = 0.5))
p2
...
```{r}
fin_gvhd <- here(path_data, 'Alpha_GVHD_Final.txt')
fout_gvhd <- path_target("Alpha-InvSimpson-5A-TALL.svg")
...
p2<-p1+stat_compare_means(comparisons=my_comparisons, label="p.signif", label.x = 1.5, label.y = 25)+theme(axis.text.x=element_text(family="Arial",colour="black",angle = 45,vjust = 0.15,hjust = 0.5))
p2
p3<- ggpar(p2, ylim = c(0, 30))
...
fout_time <- path_target("Alpha-InvSimpson-Timenew.svg")
...
otu <- otu %>%
  ...
  mutate(timepoints_show = str_replace(Timepoints, "^.*Day", "d"))
unique(otu$timepoints2)
p0 <- ggplot(otu,aes(x=timepoints2,y=InvSimpson))+
  #geom_point(aes(color=Center))+
  geom_point(position=position_jitter(h=2, w=2),alpha = 0.2, size = 1)+
  geom_smooth(aes(color=Center), se = F)+
  geom_smooth(color="black")+
  scale_x_continuous(breaks= otu$timepoints2,
                   labels=otu$timepoints_show)+
  theme_classic()+
...
p2
p3<- ggpar(p2, ylim = c(0, 30))
ggsave(fout_time, p3, width = 800, height = 800, units = "px")
p3


# Richness
p0 <- ggplot(otu,aes(x=timepoints2,y=Richness))+
  #geom_point(aes(color=Center))+
  geom_point(position=position_jitter(h=2, w=2),alpha = 0.2, size = 1)+
  geom_smooth(aes(color=Center), se = FALSE)+
  geom_smooth(color="black")+
  scale_x_continuous(breaks= otu$timepoints2,
                   labels=otu$timepoints_show)+
  theme_classic()+
  theme(legend.position = "none",panel.grid.major = element_blank(),panel.grid.minor = element_blank())
p0
p1<-p0+color_palette(c("#FFB562", "#F87474"))+labs( y = "Richness",x="")
p1
p2<-p1+
  theme(axis.text.x=element_text(family="Arial",colour="black",angle = 45, vjust = 0.3,hjust = 0.5),axis.text.y=element_text(family="Arial",colour="black"))
p2
p3<- ggpar(p2, ylim = c(0, 4000))
ggsave(path_target("Alpha-Richness-Timenew.svg"), p3, width = 800, height = 800, units = "px")
p3
```
